...
runners.filter(function (large) {
  if (large.shirt_size === 'L') {
    return runnersLargeSizeShirt.push(large);
  }
})
...
let ticketPriceTotal = runners.reduce((accum, total) => accum + total.donation,0);
...
// Problem 1 -- Return all the runners info with yahoo.com email addresses. Use .filter() //
let yahooEmail = [];
runners.filter(email => {if (email.email.includes('yahoo')) yahooEmail.push(email)});
console.log(yahooEmail);

// Problem 2 -- Alphabetize by last name all the runners. Use .forEach() //
let abcSort = [];
runners.forEach(last => abcSort.push(last.last_name));
console.log(abcSort.sort());

// Problem 3 -- Return just the email, first, & last name of whoever donated $200 or more. Use .map() //
let bigMoney = [];
runners.map(money => {if (money.donation >= 200) bigMoney.push(`${money.email}, ${money.first_name} ${money.last_name}!`)});
